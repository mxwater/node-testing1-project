...
class Seasons {
   ...
   */
  next() {
    const currentSeason = this.seasons[this.currentSeasonIndex]; 
...
class Car {
  ...
  refuel(gallons) {
    const fuelNeeded = this.tankSize - this.tank; // amount of fuel needed to fill the tank

    if (gallons <= fuelNeeded) {
      this.tank += gallons; // add fuel
    } else {
      this.tank = this.tankSize; // fill to maximum capacity
    }

    return this.tank * this.mpg; // return the miles that can be driven with the current fuel
  }
}
